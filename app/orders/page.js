    ...
    <main className="min-h-screen w-full bg-gradient-to-br from-gray-50 via-blue-100 to-white py-8">
      <div className="max-w-2xl mx-auto px-4">
        <div className="mb-8 text-center">
          <h1 className="text-3xl md:text-4xl font-extrabold text-blue-800 tracking-tight mb-2">PT Kwalram Orders</h1>
          <p className="text-lg text-blue-500 font-medium">Your order history for textile products</p>
        </div>
        {orders.length === 0 ? (
          <div className="bg-white/80 rounded-lg shadow p-6 text-center">
            <p className="text-blue-400">No orders found</p>
          </div>
        ) : (
          <div className="space-y-4">
            {orders.map((order) => (
              <div key={order.id} className="bg-white/90 rounded-xl shadow overflow-hidden border border-blue-100">
                <div className="p-6">
                  ...
                  <div className="border-t border-blue-100 -mx-6 px-6 py-4">
                    <ul className="divide-y divide-blue-50">
                      {order.items.map((item, index) => (
                        <li key={index} className="py-3 flex justify-between">
                          <div>
                            <p className="text-sm font-medium text-blue-800">{item.name}</p>
                            <p className="text-xs text-blue-500">{item.quantity}x @ Rp {item.price.toLocaleString('id-ID')}</p>
                          </div>
                  ...
                  <div className="border-t border-blue-100 -mx-6 px-6 py-4">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-xs text-blue-400">Payment Method</p>
                        <p className="text-sm font-medium text-blue-700">
                          {order.paymentMethod === 'cash' ? 'Cash' : 'QRIS'}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-blue-400">Total Payment</p>
